...
st.write(
    f"{len(rhr_df)} days of data collected. RHR is calculated using the lowest 30 minute average in a 24 hour period."  # noqa: E501
)
...
col4.metric("All Time RHR", f"{all_time_rhr:.1f} bpm", delta_color="inverse")

# Scatterplot + weekly and monthly rolling means
rhr_scatterplot = (
    alt.Chart(rhr_df)
    .mark_point(color="#FFFFFF", size=2)
    .encode(
        x=alt.Y(
            "yearmonthdate(date):T",
            title="",
            axis=alt.Axis(format="%m-%Y", tickCount="month", grid=True),
        ),
        y=alt.Y(
...
rhr_df["Weekly Mean"] = rhr_df.resting_heart_rate.rolling(
    7, min_periods=0, center=True
).mean()
rhr_df["Monthly Mean"] = rhr_df.resting_heart_rate.rolling(
    30, min_periods=0, center=True
).mean()

rolling_mean_chart = (
    alt.Chart(
        rhr_df.drop(columns="resting_heart_rate").melt(
            "date", var_name="rolling", value_name="resting_heart_rate"
        )
    )
    .mark_line(color="#3B97F3")
    .encode(
        x=alt.X("date", title=""),
...
st.altair_chart(
    rhr_scatterplot + rolling_mean_chart, use_container_width=True, theme="streamlit"
)

# Density plot
rhr_histogram = (
    alt.Chart(rhr_df)
    .mark_bar(color="#3B97F3")
    .encode(
        x=alt.X(
...
# percentile_hr = hr_df.groupby(hr_df.date.dt.date).hr.quantile(q=np.arange(0, 1.1, .1)).unstack()  # noqa: E501
# percentile_hr.columns = [f"{decile*100:.0f}-{decile*100+10:.0f}%" for decile in percentile_hr.columns]  # noqa: E501
