...
def load_dividend_data():
    ...
    df = pd.read_csv(io.StringIO(csv.getvalue().decode("utf-8")))

    df = df.dropna(subset=["Transaction Date"])
    df = df[df.Type == "Dividend"]
    df = df[(df.Portfolio == "Stocks") | (df.Portfolio == "TR")]
    df["Portfolio"] = df.Portfolio.map({"TR": "ETF", "Stocks": "Stocks"})
...
if selected_scale == "Monthly":
    dividends_trend = (
        alt.Chart(dividends.set_index("Date").resample("1M").sum().reset_index())
        .mark_line(color="#FFFFFF")
        .transform_window(
            rolling_mean="sum(Dividends)", frame=[-11, 0]  # 12 month average
        )
        .encode(
            x=alt.X(
                f"{transformation}(Date):T",
    ...
    st.altair_chart(
        alt.layer(dividends_chart, dividends_trend).resolve_scale(y="independent"),
...
years = (
    dividends.Date.dt.year.sort_values(ascending=False).unique().astype(str).tolist()
)
...
with tabs[0]:
    pt = (
        dividends.assign(Year=dividends.Date.dt.year)  # Get year
        .pivot_table(
            index="Symbol",
            columns="Year",
            values=["Dividends"],
    ...
    dividends_per_stock = (
        ...
        .properties(title="Top 20 of All Time")
        .configure_title(fontSize=25, font="Lato")
    )
...
for i, year in enumerate(years, 1):
    with tabs[i]:
        dividends_ = (
            dividends[dividends.Date.dt.year == int(year)]
            .groupby("Symbol")
            .Dividends.sum()
        ...
        dividends_per_stock = (
            ...
            .properties(title=f"Top 10 of {year}")
            .configure_title(fontSize=25, font="Lato")
        )

        st.altair_chart(
            dividends_per_stock, use_container_width=True, theme="streamlit"
        )
