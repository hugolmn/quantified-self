...
revlog["id"] = pd.to_datetime(revlog.id, unit="ms", utc=True).dt.tz_convert(
    "Europe/Paris"
)
# Loading decks
...
# Get HSK data
hsk_deck_id = decks[decks.name.str.contains("Mandarin: Vocabulary::HSK")].id.iloc[0]
...
overall_metrics[0].metric(label="Cards reviewed", value=len(hsk_detailed_revlog))
...
cumulative_hsk = cumulative_hsk.assign(
    cumcount=(cumulative_hsk.sort_values(by="id").groupby("HSK").cumcount())
)
