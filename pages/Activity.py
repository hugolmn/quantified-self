...
st.altair_chart(
    steps_barplot_daily + steps_weekly_rolling_mean_plot,
    use_container_width=True,
    theme='streamlit'
)

# Hourly plot
steps_scatterplot_hourly = alt.Chart(steps_df).mark_line(interpolate="basis").encode(
    x=alt.Y(
        'hoursminutes(date):T',
        title='',
        axis=alt.Axis(format='%H:%M', tickCount='hour', grid=True)
    ),
    y=alt.Y(
        'mean(steps):Q',
        title='Hourly Steps',
